...
exports = module.exports = internals.Socket = function (ws, req, listener) {
    ...
    if (this._listener._settings.auth &&
        ...
        this.auth._initialAuthTimeout = setTimeout(() => this.disconnect('auth-timeout'), this._listener._settings.auth.timeout);
    }
...
internals.Socket.prototype.disconnect = function (reason = 'unknown') {

    console.log(JSON.stringify({ event: 'websocket-disconnect', type: 'socket', reason }));
...
internals.Socket.prototype._processReauth = async function (request) {
    ...
    try {
        ...
        Bounce.rethrow(err, 'system');
        setTimeout(() => this.disconnect('reauth')); // disconnect after sending the response to the client
        throw err;
    }
...
internals.Socket.prototype._verifyAuth = async function () {
    ...
    try {
        ...
        Bounce.rethrow(err, 'system');
        setImmediate(() => this.disconnect(`verifyAuth`));
