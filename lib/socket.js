...
exports = module.exports = internals.Socket = function (ws, req, listener) {
    ...
    if (this._listener._settings.auth &&
        ...
        this.auth._initialAuthTimeout = setTimeout(() => this.disconnect('_initialAuthTimeout'), this._listener._settings.auth.timeout);
    }
...
internals.Socket.prototype.disconnect = function (reason) {

    console.log(JSON.stringify({ event: 'log', tags: ['nes', 'socket', 'disconnect'], data: { reason }}))
...
internals.Socket.prototype._onMessage = async function (message) {
    ...
    try {
        ...
        Bounce.rethrow(err, 'system');
        this.disconnect(`onMessage error: ${err}`);
    }
...
internals.Socket.prototype._processReauth = async function (request) {
    ...
    try {
        ...
        Bounce.rethrow(err, 'system');
        setTimeout(() => this.disconnect(`reauth error: ${err}`)); // disconnect after sending the response to the client
        throw err;
    }
...
internals.Socket.prototype._verifyAuth = async function () {
    ...
    try {
        ...
        Bounce.rethrow(err, 'system');
        setImmediate(() => this.disconnect(`verifyAuth error: ${err}`));
