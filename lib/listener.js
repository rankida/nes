...
exports = module.exports = internals.Listener = function (server, settings) {
    ...
    this._wss.on('connection', (ws, req) => {
        ...
        ws.on('error', (err) => console.log(JSON.stringify({ event: 'websocket-error', type: 'ws', err })));
        ...
        if (this._stopped ||
            ...
            console.log(JSON.stringify({
                ...
                maxConnections: `${this._settings.maxConnections}`,
                socketsLength: this._sockets.length()
            }));
    ...
    this._wss.on('error', (err) => console.log(JSON.stringify({ event: 'websocket-error', type: '_wss', err })));
...
internals.Listener.prototype._close = async function () {
    ...
    await Promise.all(Object.keys(this._sockets._items).map((id) => this._sockets._items[id].disconnect('Listener._close')));
    ...
    if (this._heartbeat &&                              // Skip the first time
        ...
        this._beatTimeout = setTimeout(() => {
            ...
            this._sockets._forEach((socket) => {
                ...
                if (!socket._active()) {
                    socket.disconnect('socket inactive (beat)');
                    return;
                }
...
internals.Sockets.prototype.auth = function (socket) {
    ...
    if (this._listener._settings.auth.maxConnectionsPerUser &&
        ...
        console.log(JSON.stringify({
            event: 'WS_CLOSING',
            type: 'maxConnectionsPerUser',
            maxConnectionsPerUser: this._listener._settings.auth.maxConnectionsPerUser,
            byUserLength: this._byUser[user].length
        }));
