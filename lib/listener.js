...
exports = module.exports = internals.Listener = function (server, settings) {
    ...
    this._wss.on('connection', (ws, req) => {
        ...
        if (this._settings.onCreation) {
            this._settings.onCreation(ws, req);
        }
        ...
        if (this._stopped ||
            ...
            if (this._settings.onMaxConnections) {
                this._settings.onMaxConnections(ws, req, this._sockets.length())
            }
    ...
    if (this._heartbeat &&                              // Skip the first time
        ...
        this._beatTimeout = setTimeout(() => {
            ...
            this._sockets._forEach((socket) => {
                ...
                if (!socket._active()) {
                    socket.disconnect('Listener._beat - not active');
                    return;
                }
